...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=19203255902:+1 (920) 325-5902\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `oiee @${num.split('@')[0]}\ seja bem vindo/a ao grupo *${mdata.subject}* comporta-se para nao ser chutado`
				...
				teks = `sayonara pessoa @${num.split('@')[0]} quem sabe possa voltar ou não ksks`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬🤡❭ AGUARDE  ENQUANTO ESTOU PROCESSANDO SE NÃO FOR TENTE NOVAMENTE ❬🤡❭',
				success: '️❬ ✔ ❭ PRONTINHO 🖤',
				error: {
					stick: 'deu erro mn',
					Iv: 'link ta inválido'
				...
				only: {
					...
					ownerG: '❬❗❭ COMANDO SÓ PODE SER USADO PELO SAD BOY❬❗❭ ',
					ownerB: '❬❗❭ COMANDO SÓ PODE SER USADO PELO SAD BOY ❬❗❭  ',
					admin: ' ❬ 😂 ❭ ESSA É BOA MEMBRO COMUM TENTANDO USAR COMANDO DE ADM KKKKKK ❬ 😂 ❭ ',
...
case 'lofi':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️Foto única de lofi mn'})
					break
                 case 'logoph':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
...
client.updatePresence(from, Presence.composing) 
			...
			tels = body.slice(6)
             ...
             break
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia cambada acorda ai bando de baianow'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde coroi'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/yOFxSUR.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa noite 😎'})
					break
		        case 'gay':		
				...
				case 'rize':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					client.sendMessage(from, nye, image, { caption: 'rize', quoted: mek })
					await limitAdd(sender) 	
					break      
				case 'sasuke':
					reply(mess.wait)
      ...
      break
        ...
        case 'delete':
              ...
              options = {
          text: `esse é o dono do grupo : wa.me/${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           }
           frhan.sendMessage(from, options, text, { quoted: mek } )
				break
                 case 'logo3d':
              	    if (args.length < 1) return reply('qual txt mn')
                    teks = `${body.slice(8)}`
			...
			case 'mapa': 
				...
				case 'figurinha':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`sapoha ${prefix}sticker e pra fazer fig com foto/gif\n mande uma imagem ou gif com a legenda !sticker ou !fig\n para fazer figurinha com vídeo é só diminuir a duração do vídeo para 5 segundos se não for é só diminuir a duração para 3 segundos\n digite !help para saber mais sobre o bot`)
					}
					break
				case 'gtts':	
				case 'tts':
       ...
       client.sendMessage(from, 'Este é o número do meu criador 🙃 ',MessageType.text, { quoted: mek} )
           ...
           case 'mudarnome':
                ...
                client.sendMessage(from, 'descriação trocada cm sucesso', text, {quoted: mek})
                break
           case 'demote':
           case 'rebaixar':
					if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Não mamou o bonde direitinho perde adm ksks :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Não mamou o bonde direitinho @${mentioned[0].split('@')[0]}perde adm ksks `, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
				case 'promover':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque a pessa para dar adm')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `🤡novo adm na área porra peita e fica sem peito🤡 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`🤡novo adm na área porra @${mentioned[0].split('@')[0]} peita e fica sem peito🤡`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
			...
			case 'remover':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `😎Deu mole e vapo removi mermo foda-se😎 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`😎Deu mole e vapo removi mermo @${mentioned[0].split('@')[0]} foda-se😎`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'converter':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('não da pra converter figurinha animada pow')
						buffer = fs.readFileSync(ran)
				...
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' *pronto mn?') 
					...
					try {
						...
						mentions(`Roubei a ft de perfil desse lek @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Putz mn deu erro🙁')
					}
					break
				case 'wait':
				case 'pesquisar':
